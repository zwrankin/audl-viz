...
def make_player_indicators(return_df=False, save_df=True):
    ...
    df_wide = df_all.groupby(player_index_vars)[player_sum_indicators].sum().reset_index()
    # Aggregates:
    ...
    # Points played (NOTE - this index assume 'player' is last item to player_index_vars
    df_points = df.groupby(player_index_vars[:-1]).apply(PlayStatsByPlayer).reset_index().drop('level_2', axis=1)
    df_points = calculate_conversion_rates(df_points)
    df_wide = pd.merge(df_wide, df_points, on=player_index_vars)
